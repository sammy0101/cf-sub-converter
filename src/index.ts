...
<head>
  ...
  <style>
    :root { 
      ...
      --accent: #38bdf8; --accent-hover: #0ea5e9; --border: #334155; --success: #22c55e; --danger: #ef4444;
      --card-hover: #2d3a52;
    }
  ...
  <div class="container">
    ...
    <div class="fav-section">
      ...
      <div id="favList" class="fav-list">
        <!-- JS 會渲染列表 -->
        <span style="color:#94a3b8; font-size:0.9rem;">暫無收藏，請在上方輸入並儲存...</span>
      </div>
    </div>
  ...
  <script>
    ...
    function generate() {
      ...
      new QRCode(qrContainer, {
        text: final, width: 180, height: 180,
    ...
    function copyUrl() {
      ...
      copyText.select();
      navigator.clipboard.writeText(copyText.value).then(() => showToast('✅ 複製成功！'));
    }
...
async function toClashWithTemplate(nodes: ProxyNode[]) {
  const resp = await fetch(`${REMOTE_CONFIG.clash}?t=${Date.now()}`, { headers: { 'Cache-Control': 'no-cache' } });
