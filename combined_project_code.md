...
Generated on: Wed Sep  9 17:13:34 UTC 2026
...
export async function toSingBoxWithTemplate(nodes: ProxyNode[], env?: Env, forceRefresh = false): Promise<string> {
  ...
  if (config.dns) {
    ...
    // 💥 解決 ECH DNS 死鎖問題：提取所有節點的 ECH 查詢網域，掛載專屬直連 DoH 解析 Type 65 記錄
    ...
    if (echDomains.length > 0) {
      if (!config.dns.servers.some((s: Record<string, unknown>) => s.tag === 'direct-doh')) {
